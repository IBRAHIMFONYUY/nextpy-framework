...
class CleanPSXParser:
    ...
    def parse(self, src: str, context: Dict[str, Any] = None) -> 'CleanPSXElement':
        """Parse PSX source to PSXElement tree"""
        context = context or {}
        ...
        return root or CleanPSXElement("div", {}, ["No content"])
    ...
    def _handle_tag_token(self, token: Token, stack: List['CleanPSXElement'], context: Dict[str, Any]) -> 'CleanPSXElement':
        ...
        if value.startswith("</"):
            ...
            if value.endswith("/>") or value in ["<br>", "<hr>", "<img>", "<input>", "<meta>", "<link>"]:
                ...
                props = {}
                if props_str:
                    # Simple prop parsing - just handle class="..." for now
                    import re
                    prop_matches = re.findall(r'(\w+)="([^"]*)"', props_str)
                    for prop_name, prop_value in prop_matches:
                        props[prop_name] = prop_value
                
                node = CleanPSXElement(tag_name, props, [])
                stack.append(node)
...
class CleanPSXElement(PSXElement):
    """Enhanced PSXElement with clean expression evaluation"""
    
    def to_html(self, context: Dict[str, Any] = None) -> str:
        """Convert to HTML with proper expression evaluation"""
        context = context or {}
        ...
        children_str = self._build_children_string(context)
        ...
        for child in self.children:
            if isinstance(child, PSXElement):
                ...
                if child.startswith('{') and child.endswith('}'):
                    # Evaluate expression
                    try:
                        expr = child[1:-1].strip()
                        if expr.startswith('for ') or expr.startswith('if ') or expr.startswith('python:'):
                            # Handle Python logic blocks
                            from .python_logic import process_python_logic
                            processed = process_python_logic(f'{{{expr}}}', context)
                            children_parts.append(processed)
                        else:
                            # Simple expression evaluation
                            evaluated = str(eval(expr, {"__builtins__": {}}, context))
                            children_parts.append(evaluated)
                    except Exception as e:
                        children_parts.append(f'{{expr error: {expr}}}')
                else:
                    children_parts.append(child)
            else:
...
# Enhanced PSXElement with clean rendering
class CleanPSXElement(PSXElement):
    """Enhanced PSXElement with clean expression evaluation"""
